...
describe("client UI contracts", () => {
  test("chapter verification restores unresolved gaps before requesting another card", () => {
    ...
    assert.match(verification, /\/api\/verification\/attempt\?\$\{params\}/);
  ...
  test("Green keeps its product identity while sharing the Blue interaction system", () => {
    ...
    assert.match(html, /green-brand\.css\?v=0\.6\.13/);
    assert.doesNotMatch(html, /blue-brand\.css/);
    assert.match(brandCss, /--blue-cobalt: #059669;/);
    assert.match(brandCss, /--blue-cobalt: #84f0c8;/);
    assert.match(electronMain, /const PREFERRED_PORT = 4557;/);
    assert.match(electronMain, /const GH_REPO = "spiral-buddy-green";/);
  });

  test("repository display helper exists before the learning home uses it", () => {
    assert.match(
      app,
      /function displayRepoName\(name\) \{\s*return displayRoadmapName\(name\);\s*\}/,
    );
    assert.match(
      app,
      /function formatRepoDisplayName\(repoName\)[\s\S]*?replace\(\/-distilled\$\/i, ""\)/,
    );
    assert.match(
      app,
      /function renderLearningHub[\s\S]*?displayRepoName\(activeRoadmap\.name\)/,
    );
    assert.match(
      app,
      /function updateTopbar[\s\S]*?displayRepoName\(activeRoadmap\.name\)/,
    );
    assert.match(
      app,
      /function renderRoadmapSelector[\s\S]*?displayRepoName\(active\.name\)/,
    );
  });
