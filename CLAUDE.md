...
- ✅ AI 프롬프트 전반 phronesis 조정 (v0.3.0) — SESSION_SYSTEM(메커니즘→반례→판단 규칙 학습법),
  노트 8섹션(핵심 원리/사례·반례/판단 규칙), Quiz 4단계(원리/적용/반례·경계/판단 시나리오),
  Look-up deep 포맷(코드 예시→사례·한계), 미리보기 예시. ⚠ 노트 섹션 헤딩 바꿀 땐
  note-writer.ts REQUIRED_SECTIONS(검증)와 STRUCTURE_SYSTEM 둘 다 + README 동기화

## Green 전용 기능 (v0.4.x — Blue에 없음)

판단 루프: 미리보기 decision 필드 → 세션 가드 → 자동 태깅 → 규칙 인덱스 → Synthesis 횡단 소환.

- **판단 규칙 인덱스** (v0.4.0): `GET /rules` (note-writer의 extractSectionBody/splitRuleItems로
  "## 판단 규칙" 섹션 추출, curated-domains.json으로 repo→레이어 매핑) + 사이드바 버튼/모달
- **세션 종료 가드** (v0.4.1): app.js endSession 앞 shouldOfferRuleDistill — 대화에 "판단 규칙"
  문자열 없으면 압축 턴 제안. 이동 중단 플로우에는 의도적 미적용 (다이얼로그 중첩)
- **Synthesis 모드** (v0.4.2): routes.ts /session/start — -everywhere 레포면 본질 모델 태그
  (incentives/compounding/feedback-loops/leverage, note-writer가 자동 부여) 노트 최대 4개 related 주입
- **미리보기 decision 필드** (v0.4.3): ChapterPreviewCard.decision — 구버전 캐시는
  decision===undefined면 miss 처리로 자동 재생성
