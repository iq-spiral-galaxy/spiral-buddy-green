...
async function main() {
  ...
  // Electron workspace가 명시한 sub-dir를 우선하고, 독립 MCP 실행 시에는
  // Green 전용 기본 폴더를 사용한다.
  const vaultSubDir = getSpiralVaultSubDir();
  ...
  server.registerTool(
    "spiral_list_roadmaps",
    {
      ...
      description:
        ...
        "2. **Curated** — GitHub 조직(예: iq-phronesis-lab)의 public 레포 중 이미 설치된 것\n\n" +
  ...
  server.registerTool(
    ...
    async ({ roadmap_id, chapter_id }) => {
      ...
      if (nextDepth === 1) {
        ...
        lines.push(
          `**Depth ${nextDepth} 복귀** — 아래 이전 노트의 '헷갈렸던 / 확인이 필요한 지점'을 진입점으로 삼으세요. 학습자가 이전 세션에서 막혔던 곳부터 다시 찌르고, depth 2는 반례·경계 조건 사냥, depth 3+는 판단 규칙 압축과 시나리오 적용까지 밀어붙이세요.`,
        );
      }
