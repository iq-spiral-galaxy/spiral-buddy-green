...
function formatRepoDisplayName(repoName) {
  const raw = cleanUiLabel(repoName).trim();
  if (!raw) return raw;
...
function _renderChapterAiCardBody(pop, chapter, card) {
  ...
  const decisionHtml = card.decision
    ? `<div class="chapter-ai-decision"><span class="chapter-ai-decision-label">내릴 수 있는 판단</span> ${_renderAiCardMarkdown(card.decision)}</div>`
    : "";
...
async function refreshUpdateBanner({ force = false } = {}) {
  ...
  // releases link 기본값
  const RELEASES_URL =
    "https://github.com/iq-spiral-galaxy/spiral-buddy-green/releases/latest";
  if (releasesLink) {
...
/**
 * 설정 모달 안에서 iq-phronesis-lab 31개를 한 번에 받기 + 워크스페이스로 자동 등록.
...
// roadmapRoot가 "/Users/x/spiral/iq-phronesis-lab"이면 부모는 "/Users/x/spiral".
...
async function runCuratedInstall(repoNames, label) {
  ...
  if (!res?.ok) {
    ...
    if (!matches && window.spiralSettings) {
      if (
        confirm(
          `iq-phronesis-lab을 워크스페이스로 등록할까요?\n${targetDir}\n(이미 등록되어 있다면 패스하세요)`,
        )
      ) {
        const wsRes = await window.spiralSettings.addWorkspace({
          name: "iq-phronesis-lab",
          sourceKind: "dir",
          localPath: targetDir,
        });
        if (wsRes?.ok) {
...
function renderRules(data, query) {
  ...
  for (const { domain, blocks } of groups.values()) {
    ...
    html.push(
      domain
        ? `<div class="rules-domain-head" style="color:${escapeAttr(domain.color)}">${categoryIconHtml(domain)}<span>${escapeHtml(cleanUiLabel(domain.name))}</span><span class="rules-domain-count">${count}</span></div>`
        : `<div class="rules-domain-head">${uiIconHtml("repo")}<span>기타</span><span class="rules-domain-count">${count}</span></div>`,
    );
      ...
      for (const item of items) {
        html.push(
          `<div class="rule-item" style="border-left-color:${escapeAttr(color)}">` +
            `<div class="rule-text">${renderMarkdown(cleanUiLabel(item))}</div>` +
